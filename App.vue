  ...
  <div>
    ...
    <form v-on:submit.prevent="submit">
      ...
      <select name="course" id="course" v-model="course" required>
        ...
        <option value="comp163">COMP 163</option>
      </select>
      <br />
      <br />
      ...
      <div class="feedback" v-for="(question, index) in questions" :key="index">
        ...
        <div
          class="option"
          v-for="(option, count) in question.options"
          :key="count"
        >
          <input
            type="radio"
...
export default {
  ...
  methods: {
    submit() {
      ...
      stringResponses = stringResponses
        .replace(/"([^"]+)":/g, "$1:")
        .replace(/\uFFFF/g, '\\"');
    ...
    fetchQuestionQuery() {
      return this.fetchGraphQL(this.questionOperationsDoc, "QuestionQuery", {});
    },
    ...
    executeResponsesMutation(mutationOperationsDoc) {
      return this.fetchGraphQL(mutationOperationsDoc, "ResponsesMutation", {});
    },
    async startExecuteResponsesMutation(mutationOperationsDoc) {
      const { errors } = await this.executeResponsesMutation(
      ...
      alert(
        "Success, please complete this form for each of your group members (including yourself)."
      );
      location.reload();
    },
    fetchUserQuery() {
      return this.fetchGraphQL(this.userOperationsDoc, "UserQuery", {});
    },
    async startFetchUserQuery() {
      ...
      for (const user of sortedUsers) {
        ...
        } else if (user.course === "comp163") {
          this.comp163Users.push(user);
        }
      }
    },
  },
